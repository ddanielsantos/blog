...
function extractFrontmatterAndBody(fileContent) {
  ...
  if (!match) {
    throw new Error(`Invalid markdown format: missing frontmatter (content preview: ${normalized.substring(0, 50)})`);
  }
  return {
    frontmatter: match[1],
    body: match[2],
...
export default function signPostsIntegration() {
  return {
    name: 'astro-sign-posts',
    hooks: {
      'astro:build:start': async () => {
        ...
        console.log('✅ Post signing complete\n');
      },
    },
  };
}
